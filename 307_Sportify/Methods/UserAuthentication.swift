...
class UserAuthentication: ObservableObject {
    ...
    func createUser(withEmail email: String, password: String, fullname: String, privateAccount: Bool, zipCode: String) async throws -> Bool {
        do {
            ...
            let user = User(id: result.user.uid, name: fullname, email: email, radius: 1, zipCode: zipCode, sportsPreferences: Set<Int>(), privateAccount: privateAccount, profilePicture: String(), age: 20, birthday: Date(), friendList: Set<String>(), blockList: Set<String>(), eventsAttending: Set<String>(), eventsHosting: Set<String>())
