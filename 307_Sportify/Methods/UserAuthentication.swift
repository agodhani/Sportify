...
class UserAuthentication: ObservableObject {
    ...
    func createUser(withEmail email: String, password: String, fullname: String, privateAccount: Bool, zipCode: String) async throws -> Bool {
        do {
            let result = try await Auth.auth().createUser(withEmail: email, password: password)
            self.userSession = result.user
<<<<<<< Updated upstream
            let user = User(id: result.user.uid, name: fullname, email: email, radius: 1, zipCode: zipCode, sportsPreferences: Set<String>(), privateAccount: privateAccount, profilePicture: Data.init(), age: 20, birthday: Date(), friendList: Set<String>(), blockList: Set<String>(), eventsAttending: Set<String>(), eventsHosting: Set<String>())
=======
            let user = User(id: result.user.uid, name: fullname, email: email, radius: 1, sportsPreferences: Set<String>(), privateAccount: true, profilePicture: Data.init(), age: 20, birthday: Date(), friendList: Set<String>(), blockList: Set<String>(), eventsAttending: Set<String>(), eventsHosting: Set<String>())
>>>>>>> Stashed changes
